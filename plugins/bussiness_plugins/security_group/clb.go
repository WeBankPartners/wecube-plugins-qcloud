...
import (
	...
	clb "github.com/tencentcloud/tencentcloud-sdk-go/tencentcloud/clb/v20170317"
)
...
func createClbClient(providerParams string)(client *clb.Client,err eror){
...
func (resourceType *ClbResourceType)QueryInstancesById(providerParams string, instanceIds []string) (map[string]ResourceInstance, error){
	...
	var offset,limit int64 = 0,len(instanceIds)
...
func (resourceType *ClbResourceType)QueryInstancesByIp(providerParams string, ips []string) (map[string]ResourceInstance, error){
	...
	var offset,limit int64 = 0,len(ips)
	region,_:=plugins.GetRegionFromProviderParams(providerParams)
...
func (instance ClbInstance)GetBackendTargets(providerParams string,proto string,port string)([]ResourceInstance,error){
	...
	//先获取监听器
	listenerId,err:=getLbListener(providerParams,instance.Id,forward,proto,port)
	if err != nil {
		return instances,fmt.Errorf("can't found listener,lb=%s,proto=%s,port=%s",instance.Id,proto,port)
	}

	//获取后端RS
	instances,err:=getLbBackendTargets(providerParams,instance.Id,forward,listenerId,instance.Vip)
	if err != nil {
...
func getAppLbListenerId(client *clb.Client,lbId string,proto string,port int64)(string,error){
	 ...
	 resp,err:=clb.DescribeListeners(request)
	 if err != nil {
		 return "",err 
	 }
...
func getClassicLbListenerId(client *clb.Client,lbId string,proto string,port int64)(string,error){
	 ...
	 resp,err:=clb.DescribeClassicalLBListeners(request)
	 if err != nil {
		 return "",err 
	 }
...
func getLbListener(providerParams string,id string,forward uint64,proto string,port string)(string,error){
	listenerId:= ""
	client,_:=createClbClient(providerParams)
	...
	if forward == 1 {
		listenerId,err=getAppLbListenerId(client,instance.id,proto,portInt64)
	}

	//classic 
	if forward == 0 {
		listenerId,err=getClassicLbListenerId(client,instance.id,proto,portInt64)
	}
...
func getAppLbBackends(client *clb.Client,lbId string,listenerId string)([]string,error){
	...
	for _,target := range resp.Response.Listeners[0].Targets{
		instanceIds:=apend(instanceIds,*target.InstanceId)
	} 
...
func getLbBackendTargets(providerParams string,id string,forward uint64,listenerId string,vip string)([]ResourceInstance,error){
	...
	if forward == 1 {
		instanceIds,err=getAppLbBackends(client,instance.id,listenerId)
	}

	if forward == 0 {
		instanceIds,err=getClassicLbBackends(client,instance.id,listenerId)
	}
